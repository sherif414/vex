      ...
      h(
        "div",
        {
          ...
          style: {
            ...
            "min-width": "var(--vex-auto-min-width)",
          },
        },
        slots.default?.()
      );
...
export const ComboboxListbox = defineComponent<ComboboxListboxProps>(
  (p, { slots, emit }) => {
    const { listboxID, triggerEl, triggerID, listboxEl } =
...
export const ComboboxOption = defineComponent<ComboboxOptionProps>(
  (p, { slots }) => {
    ...
    const item = collection.addItem(optionEl, () => !!p.disabled);
    onUnmounted(() => collection.removeItem(optionEl));
